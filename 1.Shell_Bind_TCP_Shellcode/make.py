...
parser.add_argument("filename", help="Assembly file to change port")
...
def replace_port_in_assembly_file(filename, old_string, new_string):
...
print "[i] Port number ready to copy in shellcode.c (format \\xAB\\xCD) ...: \t\t"+ port_for_shellcode_c

port_for_shellcode_nasm = convert_port_to_hex_for_assembly_file()
print "[i] Port number ready to copy in the assembly program (format 0xCDAB) ...: \t"+ port_for_shellcode_nasm

print "[+] Patching assembly ("+ args.filename+ ") file with new port ...: \t\t\t"+ str(args.port)
replace_port_in_assembly_file(args.filename, "0x5c11", port_for_shellcode_nasm)
#replace_port_in_assembly_file(args.filename, "0xW00T", port_for_shellcode_nasm)
